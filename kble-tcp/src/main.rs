...
async fn main() -> io::Result<()> {
    ...
    println!("WebSocket server listening on ws://{}:{}",args.ws_addr,args.ws_port);
    ...
    loop {
        let (stream, _) = ws_listener.accept().await.expect("Failed to accept connection");
        ...
        tokio::spawn(async move {
            ...
            if *is_client_connected {
                ...
                *is_client_connected = true;
                ...
                handle_ws_client(stream, Arc::clone(&client_connected), Arc::clone(&tcp_upstream), Arc::clone(&tcp_downstream)).await;
            }
        });
    }
}

async fn handle_ws_client(ws_stream: TcpStream, client_connected: Arc<Mutex<bool>>, tcp_upstream: Arc<Mutex<tokio::io::ReadHalf<TcpStream>>>, tcp_downstream: Arc<Mutex<tokio::io::WriteHalf<TcpStream>>>) {
    let ws_stream = accept_async(ws_stream).await.expect("Error during WebSocket handshake");
    ...
    let tcp_to_ws = async {
        ...
        loop {
            ...
            match tcp_stream.read(&mut buffer).await {
                ...
                Ok(n) => {
                    if ws_write.send(Message::binary(buffer[..n].to_vec())).await.is_err() {
